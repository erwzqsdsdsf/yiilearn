...
class CWebUser extends CApplicationComponent implements IWebUser
{
	...
	{
		$id=$identity->getId();
		$states=$identity->getPersistentStates();//这里是从用户ident里面来的状态。
	...
	protected function loadIdentityStates($states)
	{
		...
		if(is_array($states))
		{
			...
			{
				$this->setState($name,$value);//这个是关键，把从用户ident来的固化状态，添加到线程了
				$names[$name]=true;//这里对专用项，也就是储存到cookies里面的，添加信息。
			}
		}
